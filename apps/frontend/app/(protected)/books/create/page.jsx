...
export default function CreateBookPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to fetch authors/publishers", err);
      }
    };
    fetchData();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(releaseYearInt)) {
      return setError("Invalid release year");
    }

    try {
      await api.books.create(title, authorId, publisherId, Number(releaseYear));
      router.push("/books");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-8">
        <h1 className="text-3xl font-bold mb-6 text-black/80">Create Book</h1>
